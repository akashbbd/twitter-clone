...
function CreateTweetForm({ onCreate }) {
  ...
  function validateTweet(tweet) {
    if (tweet.text.length < 1 || tweet.text.length > 140) {
      setErrors({
        message: 'Tweet text length must be between 1 and 140 characters',
      });
      return false;
    }
    return true;
  ...
  return (
    <Form onSubmit={addTweet}>
      ...
      <Button
        ...
        type="submit"
        disabled={
          text.length < 1 || text.length > 140 || createTweetMutation.isLoading
        }
      >
